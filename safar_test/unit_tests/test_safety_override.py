...
def test_stopped_driver_can_reverse_away_during_critical_risk():
    reverse = ManualControl(
        throttle=0.45,
        reverse=True,
        manual_gear_shift=True,
        gear=-1,
    )
    result = SafetyOverride().apply(reverse, decision(ActionType.EMERGENCY_BRAKE))
    assert result.control == reverse
    assert not result.active
